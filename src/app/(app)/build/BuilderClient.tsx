...
import { CageSumDialog } from "../../../components/CageSumDialog";
...
export default function BuilderClient({ puzzleId, initialTitle, initialConstraints }: Props) {
  ...
  useEffect(() => {
    setMode("build");
    if (puzzleId !== null) {
      loadPuzzle(initialConstraints, puzzleId, initialTitle);
    } else {
      reset();
    }
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      const { selected, constraints: cs, draft } = store;
  ...
  }, [setCageDialogCells, setHelpOpen]);
